#!/usr/bin/env python3
"""
RQ1 workload-stability profiler for ImportanceGradient.
...
Typical use:
  torchrun --standalone --nproc_per_node=4 rq1_profile_workload_importance.py \
...
def collect_component_activity(model: torch.nn.Module, topk_ratio: float) -> List[dict]:
...
def geometric_knee_threshold(scores: np.ndarray) -> Tuple[int, float]:
    ...
    distances = np.abs(np.cross(ab, points - a) / denom)
...
def parse_args():
    ...
    # model / data
    p.add_argument("--model_name_or_path", type=str, default="facebook/opt-1.3b")
...
def main():
    ...
    while global_step < args.max_steps:
        ...
        if next_step % args.trace_every == 0:
            rows = collect_component_activity(model, topk_ratio=args.topk_ratio)
            if is_main_process():
