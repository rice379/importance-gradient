...
#   MAX_STEPS=20 MAX_TRAIN_SAMPLES=128 MAX_EVAL_SAMPLES=64 LOCAL_FILES_ONLY=0 bash run_rq1_pythia_workload_stability.sh
#
# Full run on visible GPUs:
#   bash run_rq1_pythia_workload_stability.sh
#
# Force two GPUs:
#   CUDA_VISIBLE_DEVICES=0,1 GPUS=2 bash run_rq1_pythia_workload_stability.sh
...
OUT=${OUT:-runs/rq1_workload_stability_pythia1.4b}
MODEL=${MODEL:-EleutherAI/pythia-1.4b}
MODEL_LABEL=${MODEL_LABEL:-pythia-1.4b}
SEED=${SEED:-42}
...
SPLIT_FUSED_QKV=${SPLIT_FUSED_QKV:-1}
...
if [[ "$SPLIT_FUSED_QKV" == "0" ]]; then
  MODEL_ARGS+=(--no_split_fused_qkv)
fi
...
for SPEC in "${WORKLOAD_SPECS[@]}"; do
  ...
  torchrun --standalone --nproc_per_node="$GPUS" "$SCRIPT_DIR/rq1_profile_workload_importance_pythia.py" \
...
python "$SCRIPT_DIR/rq1_analyze_workload_stability_pythia.py" \
  --profile_glob "$OUT/profile_${MODEL_LABEL}_*/profiles/profile_*.csv" \
  --output_dir "$OUT/analysis" \
  --group_by model_name

echo "[done] RQ1 Pythia workload-stability outputs: $OUT"
